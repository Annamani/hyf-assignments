...
select t.title ,s.name as status_name,u.name as User_name from task t
join status s on t.status_id=s.id
...
select t.title as task_name,u.name as user_name, s.name as status from task t
join user_task ut on ut.task_Id =t.task_id 
join user u on u.user_Id =ut.user_Id 
join status s on s.id=t.status_id 
where u.name="Aarika Ellingworth";
...
where t.title="Study SQL Queries";

--Find how many tasks each user is responsible for. Hint: COUNT(), GROUP BY
select u.name,count(ut.task_Id) as user_total_tasks from task t
Right join user_task ut on ut.task_Id =t.task_id 
right join user u on u.user_Id =ut.user_Id 
group BY u.name ;

select u.name,count(ut.task_Id) AS user_total_tasks from user u
left join user_task ut on u.user_Id =ut.user_Id 
GROUP BY u.user_Id 
ORDER BY user_total_tasks;

--Find how many completed tasks each user has. Order them in descending order.
SELECT u.name, COUNT(t.task_id) AS completed_tasks FROM user u
left join user_task ut on u.user_id = ut.user_id
left join task t on ut.task_id = t.task_id 
--If we dont know that status is done and its id is 3
--join status s on s.id =t.status_id 
--where s.name="Done"
where t.status_id = 3
GROUP BY u.name 

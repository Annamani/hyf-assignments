...
app.get("/api/stats/tasks-per-user-unoptimized", async (req, res) => {
  try {
    const tasks = await db.raw(
      "SELECT task.*, user_task.user_id FROM task INNER JOIN user_task ON task.task_id = user_task.task_id",
    );
...
app.get("/api/stats/status-distribution", async (req, res) => {
  try {
    const stats = await db.raw(
      `SELECT status.name as status, COUNT(task.task_id) as count
      FROM task
